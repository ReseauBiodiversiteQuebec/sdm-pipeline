...
load_cube <- function(stac_path =
  ...
  datetime <- format(lubridate::as_datetime(t0), "%Y-%m-%dT%H:%M:%SZ")
    ...
    datetime <- it_obj$features[[1]]$properties$datetime
  }
  if (!is.null(t1) && t1 != t0) {
  ...
  if (is.null(spatial.res)) {
    ...
    spatial.res <-  datetime <- it_obj$features[[1]]$assets[[name1]]$`raster:bands`[[1]]$spatial_resolution
  }
...
get_layers_cube <- function(stac_path =
                              "http://io.biodiversite-quebec.ca/stac/",
                            limit = 5000,
                            collections = c('chelsa-clim'),
                            use.obs = T,
  ...
  if (use.obs) {
    ...
    if (inherits(obs, "data.frame")) {
      # Reproject the obs to the data cube projection
      proj.pts <- project_coords(obs, lon = lon, lat = lat, proj.from = srs.obs)
